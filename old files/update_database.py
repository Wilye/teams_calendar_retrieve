...
def get_latest_message_date(connection):
    cursor = connection.cursor()
    
    # Query the database to get the most recent message
    cursor.execute("SELECT MAX(LastModifiedDateTime) FROM TeamsMessages")
    ...
    if result:
        formatted_date = result[0].strftime("%Y-%m-%d %H:%M:%S")
        print("max last modified date time: " + formatted_date)
        return result[0]
...
def get_messages(access_token, group_id, channel_id, last_message_date=None):
    graph_url = f"https://graph.microsoft.com/beta/teams/{group_id}/channels/{channel_id}/messages/delta"
    if last_message_date is not None:
        last_message_date_str = last_message_date.isoformat(timespec='milliseconds').replace("+00:00", "") + 'Z'
        graph_url += f"?$filter=lastModifiedDateTime gt {last_message_date_str}"
...
def insert_or_update_message_in_db(connection, message):
    cursor = connection.cursor()
    ...
    print("last modified date time of message: " + LastModifiedDateTime)
    ...
    print("cursor messages: " + str(cursor.messages))
    connection.commit()
...
def main():
    ...
    last_message_date = get_latest_message_date(connection)

    messages = get_messages(access_token, group_id, channel_id, last_message_date)
    
    for message in messages:
        insert_or_update_message_in_db(connection, message)
